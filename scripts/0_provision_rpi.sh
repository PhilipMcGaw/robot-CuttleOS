...
[[ -f "$DATALOGGER_ROOT/configs/datalogger.service" ]] || fail "Datalogger service file is missing: $DATALOGGER_ROOT/configs/datalogger.service. Restore the Datalogger component before continuing."
[[ -f "$DATALOGGER_ROOT/requirements.txt" ]] || fail "Datalogger requirements are missing: $DATALOGGER_ROOT/requirements.txt. Restore the Datalogger component before continuing."
[[ -f "$CONTROL_ROOT/configs/python.service" ]] || fail "Control service file is missing: $CONTROL_ROOT/configs/python.service. Restore the Control component before continuing."
[[ -f "$CONTROL_ROOT/requirements.txt" ]] || fail "Control requirements are missing: $CONTROL_ROOT/requirements.txt. Restore the Control component before continuing."
[[ -r "$NATS_CONFIG_FILE" ]] || fail "NATS configuration is missing: $NATS_CONFIG_FILE. Copy configs/nats.env.example to configs/nats.env and review it."
