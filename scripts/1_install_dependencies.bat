...
set "REQUIREMENTS=%PROJECT_ROOT%\apps\cockpit\requirements.txt"
...
if not exist "%REQUIREMENTS%" (
 ...
 echo [FAIL] Corrective action: restore requirements.txt from the Cockpit repository.
 exit /b 1
)
if not exist "%PYTHON%" (
...
"%PYTHON%" -m pip install --no-warn-script-location -r "%REQUIREMENTS%" || exit /b 1
