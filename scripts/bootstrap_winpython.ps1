...
foreach($series in @("3.14","3.13")){ foreach($line in Get-Content $manifest){ if($line -match "^\s*[0-9a-fA-F]{32}\s*\|\s*[0-9a-fA-F]{40}\s*\|\s*([0-9a-fA-F]{64})\s*\|\s*(WinPython64-" + [regex]::Escape($series) + "\.[0-9.]+dot(?:b[0-9]+)?\.zip)\s*\|"){ $expectedSha256=$Matches[1].ToUpperInvariant();$selectedName=$Matches[2];break } };if($selectedName){break} }
if(-not $selectedName){throw "No stable 64-bit WinPython release was found."}
Write-Host "[INFO] Selected WinPython archive: $selectedName"
$candidates=@("https://winpython.github.io/" + $selectedName)
try{ $releases=Invoke-RestMethod -Uri "https://api.github.com/repos/winpython/winpython/releases?per_page=20" -Headers @{Accept="application/vnd.github+json";"User-Agent"="robot-CuttleOS-bootstrap"}; foreach($release in $releases){ foreach($asset in $release.assets){ if($asset.name -eq $selectedName){ $candidates += $asset.browser_download_url } } } }catch{ Write-Host "[INFO] GitHub release metadata lookup failed; trying release-feed URLs." }
try{[xml]$feed=(Invoke-WebRequest -Uri "https://github.com/winpython/winpython/releases.atom" -UseBasicParsing).Content;foreach($entry in $feed.feed.entry){if($entry.link.href -match '/releases/tag/([^/?#]+)'){ $tag=[Uri]::EscapeDataString($Matches[1]); $candidates += "https://github.com/winpython/winpython/releases/download/$tag/WinPython/$selectedName"; $candidates += "https://github.com/winpython/winpython/releases/download/$tag/$selectedName" }}}catch{Write-Host "[INFO] Release feed lookup failed; using the official download page."}
$downloaded=$false; $attempted=0; foreach($candidate in ($candidates | Select-Object -Unique)){ $attempted++; Write-Host "[INFO] Download attempt $attempted from $candidate"; try{Invoke-WebRequest -Uri $candidate -UseBasicParsing -MaximumRedirection 10 -OutFile $archive;if((Get-Item $archive).Length -ge 1000000){$downloaded=$true;break}}catch{Write-Host "[INFO] Download attempt failed: $($_.Exception.Message)";Remove-Item $archive -Force -ErrorAction SilentlyContinue}}
